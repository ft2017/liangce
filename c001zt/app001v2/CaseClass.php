...
class X {
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL_是否顯示原欄位_EXT($sql, $toShowSQL = false, $toShowField = false) {
        ...
        $arr = T100PROD::getArray($sql);
        echo X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位_EXT($arr, $toShowField);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL_是否顯示原欄位($sql, $toShowSQL = false, $toShowField = false) {

        $arr = T100PROD::getArray($sql);
        echo X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位($arr, $toShowField);
        if ($toShowSQL) {
            echo "$sql";
        }
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL($sql, $toShowSQL = false) {
        ...
        $arr = T100PROD::getArray($sql);
        echo X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位($arr, $toShowSQL);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN($sql, $btn) {
        $arr = T100PROD::getArray($sql);
        echo X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN($arr, $btn);
    }
...
//var_dump($arr);
        foreach ($arr as $key => $val) {
            $seq = $val['SEQ'];
//    echo "<h1>$seq</h1>";
            X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN_BTNS用JSON字串_取FT000($app, $page, $seq);
        }
    }
...
//         $sql='';
        ...
        echo X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN_允許沒有BTN($arr, $btns);
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_客制思科組產品BTN($sql) {
        $arr = T100PROD::getArray($sql);
        echo X::getHtmlTable基本款有流水号_自动栏位_客制思科組產品BTN($arr);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_客制視圖BTN($sql) {
        $arr = T100PROD::getArray($sql);
        echo X::getHtmlTable基本款有流水号_自动栏位_客制視圖BTN($arr);
    }
...
//                echo "???".$btn['para'];
                    if (!isset($btn['para']) || $btn['para'] == null) {
                        ...
                        $x .= "<a class='btn btn-default' href='$url'>$display</a> ";
...
//            echo $alignX."  $align<hr>";
                ...
                $strTable .= X::getAlignTd($alignX, $val2);
//                switch ($alignX) {
//                    case 'L':
//
//                        $strTable .= "<td>";
...
//            $strTable .= HTML::getBtn(HTML::getAsft301Url($wo), 'btn-primary', $wo);
            $strTable .= HTML::getAsft301Link($wo);
            $strTable .= "</td>";
...
//                    $strTable .= HTML::getBtn("case_prod.php?PROD=$val2", "btn-primary", $val2);
                    $strTable .= HTML::getBtn(HTML::get產品V2工单有带RcardUrl($val2), "btn-primary", $val2);
                    $strTable .= "</td>";
                } else {
                    $strTable .= "<td>";
                    $strTable .= $val2;
...
//                    $so_yyyy_mm="2017-09";
                    ...
                    $strTable .= "<td class='text_align_right'>";
//                            $strTable .= number_format($val2);
                    $strTable .= HTML::get客户每月订单列表Link($cust, $so_yyyy_mm, number_format($val2));
//                    $strTable .= $val2;
