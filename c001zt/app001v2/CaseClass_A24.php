...
class A24X {
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位($sql) {
        ...
        echo A24X::getHtmlTable基本款有流水号_自动栏位($arr);
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL_是否顯示原欄位_EXT($sql, $toShowSQL = false, $toShowField = false) {
        ...
        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位_EXT($arr, $toShowField);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL_是否顯示原欄位($sql, $toShowSQL = false, $toShowField = false) {

        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位($arr, $toShowField);
        if ($toShowSQL) {
            echo "$sql";
        }
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL($sql, $toShowSQL = false) {
        ...
        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位($arr, $toShowSQL);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN($sql, $btn) {
        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN($arr, $btn);
    }
...
//    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數($sql, $btn) {
        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數($arr, $btn);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN($sql, $btn) {
        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN($arr, $btn);
    }
...
//        echo  $title ;
        ...
        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN_允許沒有BTN($arr, $btn);
    }

    static public function showPageTitleAndEachTables($app, $page) {
        // （一）show Page Title
        A24X::showPageTitle($app, $page);
...
//            }
            A24X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN_BTNS用JSON字串_取FT000($app, $page, $seq);
        }
    }
...
//        $sql = X::getParsedSQL($arr[0]['SQL']);
        if (!isset($arr[0]['TITLE'])) {
            ...
            A24X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN("FT000", $sql);
        } else {
            $title = $arr[0]['TITLE'];
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN_BTNS用JSON字串_取FT000($app, $page, $seq) {
        ...
        $isBPM = false;
        $BPM = '【BPM】';
        $sql = A24X::getParsedSQL($arr[0]['SQL']);
...
//        echo "<span class='badge badge-default'>$title</span>";
        ...
        echo A24X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN_允許沒有BTN($arr, $btns);
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_客制思科組產品BTN($sql) {
        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位_客制思科組產品BTN($arr);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_客制視圖BTN($sql) {
        $arr = T100PROD::getArray($sql);
        echo A24X::getHtmlTable基本款有流水号_自动栏位_客制視圖BTN($arr);
    }
...
//            echo $alignX."  $align<hr>";
                ...
                $strTable .= A24X::getAlignTd($alignX, $val2);
//                switch ($alignX) {
//                    case 'L':
//
//                        $strTable .= "<td>";
