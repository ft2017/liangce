...
class A22X {
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位($sql) {
        ...
        echo A22X::getHtmlTable基本款有流水号_自动栏位($arr);
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL_是否顯示原欄位_EXT($sql, $toShowSQL = false, $toShowField = false) {
        ...
        $arr = T100PROD::getArray($sql);
        echo A22X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位_EXT($arr, $toShowField);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL_是否顯示原欄位($sql, $toShowSQL = false, $toShowField = false) {

        $arr = T100PROD::getArray($sql);
        echo A22X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位($arr, $toShowField);
        if ($toShowSQL) {
            echo "$sql";
        }
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL($sql, $toShowSQL = false) {
        ...
        $arr = T100PROD::getArray($sql);
        echo A22X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位($arr, $toShowSQL);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN($sql, $btn) {
        $arr = T100PROD::getArray($sql);
        echo A22X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN($arr, $btn);
    }
...
//        
        ...
        foreach ($arr as $key => $val) {
            $seq = $val['SEQ'];
            A22X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN_BTNS用JSON字串_取FT000($app, $page, $seq);
        }
    }
...
//        $sql = X::getParsedSQL($arr[0]['SQL']);
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN_BTNS用JSON字串_取FT000($app, $page, $seq) {
        ...
        $arr = T100PROD::getArray($sql);
        $sql = A22X::getParsedSQL($arr[0]['SQL']);
        $title = $arr[0]['TITLE'];
...
//        var_dump($jsonStr);…
        ...
        echo A22X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN_允許沒有BTN($arr, $btns);
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_客制思科組產品BTN($sql) {
        $arr = T100PROD::getArray($sql);
        echo A22X::getHtmlTable基本款有流水号_自动栏位_客制思科組產品BTN($arr);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_客制視圖BTN($sql) {
        $arr = T100PROD::getArray($sql);
        echo A22X::getHtmlTable基本款有流水号_自动栏位_客制視圖BTN($arr);
    }
...
//                HTML::showDEBUG();
                ...
                foreach ($btns as $btn) {
                    ...
                    if (!isset($btn['para']) || $btn['para'] == null) {
                        ...
                        $x .= "<a class='btn btn-default' href='a22.php?app=case&page=$url'>$display</a> ";
...
//                            str_replace("world","Shanghai","Hello world!");
                        ...
                        $x .= "<a class='btn btn-default' href='a22.php?app=case&page=$url$strPara'>$display</a> ";
                    } else {
                        $url = $btn['url'];
                        $display = $btn['display'];
                        $strPara = "";
...
//            echo $alignX."  $align<hr>";
                ...
                $strTable .= A22X::getAlignTd($alignX, $val2);
//                switch ($alignX) {
//                    case 'L':
//
//                        $strTable .= "<td>";
...
class 每日产出 {
    ...
    public static function showBtns() {
        HTML::showBtn("day_dept_stn_out.php?show_type=0&out_date=" . DEV::getToday(), " btn-primary", "【1】报工单");
        HTML::showBtn("day_dept_stn_batch.php?show_type=0&out_date=" . DEV::getToday(), " btn-primary", "【2】批号");
        HTML::showBtn("day_dept_stn_wo.php?show_type=0&out_date=" . DEV::getToday(), "  btn-primary", "【3】工单");
        HTML::showBtn("day_dept_stn_prod.php?show_type=0&out_date=" . DEV::getToday(), " btn-primary", "【4】产品");
        echo '&nbsp';
        HTML::showBtn("day_dept_stn_out.php?show_type=1&out_date=" . DEV::getToday(), "btn-success ", "【1】报工单+");
        HTML::showBtn("day_dept_stn_batch.php?show_type=1&out_date=" . DEV::getToday(), "btn-success ", "【2】批号+");
        HTML::showBtn("day_dept_stn_wo.php?show_type=1&out_date=" . DEV::getToday(), "btn-success ", "【3】工单+");
        HTML::showBtn("day_dept_stn_prod.php?show_type=1&out_date=" . DEV::getToday(), "btn-success ", "【4】产品+");
    }
...
//        echo HTML::getColorBtn(1, HTML::get全制程查询Url(), TITLE::$全制程查询);
        HTML::showBtn(HTML::get全制程查询Url(), " btn-primary", TITLE::$全制程查询);

        HTML::showBtn("qry001_wo_rej.php?show_type=0 ", " btn-info", "（1）基础信息");
        HTML::showBtn("qry001_wo_rej.php?show_type=1 ", " btn-info", "（2）显示单据");
        HTML::showBtn("qry001_wo_rej.php?show_type=2 ", "  btn-info", "（3）工站不良说明");
    }
...
class 批次不良 {
    ...
    public static function showBtns() {
        HTML::showBtn(HTML::get全制程查询Url(), " btn-primary", TITLE::$全制程查询);
        HTML::showBtn("qry001_batch_rej.php?show_type=0 ", " btn-info", "（1）基础信息");
        HTML::showBtn("qry001_batch_rej.php?show_type=1 ", " btn-info", "（2）显示单据");
        HTML::showBtn("qry001_batch_rej.php?show_type=2 ", "  btn-info", "（3）工站不良说明");
    }
...
class 图片开关 {
    ...
    public static function showBtns() {
        HTML::showBtn("helpdesk002.php?show_type=0 ", " btn-info", "（1）只显示文字");
        HTML::showBtn("helpdesk002.php?show_type=1 ", " btn-info", "（2）显示文字和图片");
    }
...
class 不合格品处理单 {
    ...
    public static function showBtns() {
        HTML::showBtn(HTML::get不合格品处理单161Url(), " btn-primary", "【161】SF161不合格品处理单");
        HTML::showBtn(HTML::get不合格品处理单162Url(), "btn-success ", "【162】SF162数量差异调整--不走签核");
    }
...
//        echo "<h4>$text</h4>";
    ...
    static public function ztstartpage() {
...
//            $strTable .= HTML::getBtn(HTML::getAsft301Url($wo), 'btn-primary', $wo);
            $strTable .HTMLml::getAsft301Link($wo);
            $strTable .= "</td>";
...
//            $strTable .= "..." . $val['WO'];
            $wo = $val['WO'];
//            $strTable .= HTML::getBtn("case_asft301.php?WO=$WO", 'btn-primary', $WO);
            $strTable .HTMLml::getBtHTMLml::getAsft301Url($wo), 'btn-primary', $wo);
            $strTable .= "</td>";
...
//            $strTable .= "<th>" . (1 + $key) . "</th>";
            ...
            foreach ($val as $key2 => $val2) {
                ...
                if ($val2 == 'N') {
                    ...
                    $strTable .HTMLml::get按工单查报工列表Link($val2);
//                    $strTable .= HTML::getProdLink($val2);
                    $strTable .= "</td>";
                } else if ($key2 == "PROD") {
                    $strTable .= "<td>";
                    $strTable .HTMLml::get產品Link($val2);
                    $strTable .= "</td>";
                } else {
                    $strTable .= "<td>";
                    $strTable .= $val2;
...
//                    $strTable .= implode(", ", REGEX::getEnglishNumberAndChineseWords($val2));
                    ...
                    $arr = REGEX::getEnglishNumberAndChineseWords($val2);
                    foreach ($arr as $k1 => $v1) {
                        $strTable .HTMLml::getBtn("case_prodname_find.php?find=$v1", "btn-default", $v1);
                    }
...
//                    $strTable .= HTML::getBtn("case_prod.php?PROD=$val2", "btn-primary", $val2);
                    $strTable .HTMLml::getBtHTMLml::get產品V2工单有带RcardUrl($val2), "btn-primary", $val2);
                    $strTable .= "</td>";
                } else {
                    $strTable .= "<td>";
                    $strTable .= $val2;
...
//                    $so_yyyy_mm="2017-09";
                    ...
                    $strTable .= "<td class='text_align_right'>";
//                            $strTable .= number_format($val2);
                    $strTable .HTMLml::get客户每月订单列表Link($cust, $so_yyyy_mm, number_format($val2));
//                    $strTable .= $val2;
        ...
        foreach ($arr as $key => $val) {
            ...
            foreach ($val as $key2 => $val2) {
                if ($key2 == 'SO') {
                    $strTable .= "<td>";
//                    $strTable .= "XXXXXXXXXXXXXXXXXX";
                    $strTable .HTMLml::get客户订单查询Link($val2);
//                    $strTable .= "XXXXXXXXXXXXXXXXXX";
//                    $cust=$val2['CUST'];
