...
//    }
//}
//require 'CaseClass_A25_HTML.php'; // 由此帶入必要的功能
class CaseClass_A25_CONST {

    const PROJECT_CODE = 'a25';
    const APP_IS_CASE = 'app=case';
    const BTN_CLASS = "class='btn btn-primary btn-sm'";

}

class CaseClass_A25_HTML extends CaseClass_A25_CONST {
...
//class HTML {
    ...
    public static function showBtn新的頁面($url, $btnClass, $displayText) {
        $btnClassV2 = " class='btn $btnClass '";

        echo "<span style='margin-left:4px'><a $btnClassV2 target='_blank' href='$url'>$displayText</a><span>";
    }
...
//        $btnClassV2 = " class='btn $btnClass '";
//        $url = "case_prod.php?PROD=$prod";
        $url = HTML::get產品V2工单有带RcardUrl($prod);
...
//        $btnClassV2 = " class='btn $btnClass '";
//        $url = "case_asft301.php?wo=$wo";
        $url = HTML::getAsft301RcardUrl($wo, $rcard);
...
//        $btnClassV2 = " class='btn $btnClass '";
        $url = HTML::get工单入库明细Url($wo);
        return "<a href='$url'>$text</a>";
    }

    public static function get按產品查工單Link($prod, $text) {
//        $btnClassV2 = " class='btn $btnClass '";
        $url = HTML::get按產品查工單Url($prod);
        return "<a href='$url'>$text</a>";
    }
...
//        $btnClassV2 = " class='btn $btnClass '";
        $url = HTML::get客户订单查询V2Url($so, $cust, $so_yyyy_mm);
        return "<a href='$url'>$text</a>";
    }
...
//        $btnClassV2 = " class='btn $btnClass '";
        $url = HTML::getADV_CHECKing002Url($wo);
        return "<a href='$url'>$wo</a>";
    }
...
//        $btnClassV2 = " class='btn $btnClass '";
        $url = HTML::getADV工单按批号项次工序良品转出的统计表Url($wo);
        return "<a href='$url'>$wo</a>";
    }

    public static function getADV产品按工单项次工序良品转出的统计表Link($prod) {
//        $btnClassV2 = " class='btn $btnClass '";
        $url = HTML::getADV产品按工单项次工序良品转出的统计表Url($prod);
        return "<a href='$url'>$prod</a>";
    }
...
//        $btnClassV2 = " class='btn $btnClass '";
        $url = HTML::getBPM流程审批按流程至关卡Url($bpm);
...
//        $btnClassV2 = " class='btn $btnClass '";
        $url = HTML::getBPM流程审批按流程至个人Url($bpm);
        return "<a class='btn btn-default' href='$url'>$text</a>";
    }
...
//    }
    ...
    public static function getBPM流程审批个人需要签核的统计表Link($txt) {
        $url = HTML::getBPM流程审批个人需要签核的统计表Url();
        return "<a href='$url'>$txt</a>";
    }

    public static function getBPM流程审批至个人Link($empe, $txt) {
        $url = HTML::getBPM流程审批至个人Url($empe);
        return "<a href='$url'>$txt</a>";
    }
    ...
    public static function getBPM流程审批至流程及个人Link($bpm, $empe, $txt) {
        $url = HTML::getBPM流程审批至流程及个人Url($empe);
        return "<a href='$url'>$txt</a>";
    }
...
//        echo "<h4>$text</h4>";
        ...
        HTML::showH4("<a href='case_prodname.php'>產品</a>︰$text");
    }
...
//        echo "<h4><span class='dev001'>$text</span></h4>";
        HTML::showH4有灰底色("外协厂商：" . HTML::get工单Link($wo));
    }
...
//        $show_sql = MARK::initVarWithDefaultVal('show_sql', 0); //
        ...
        require A25X::PROJECT_CODE . '_HTML_START.php';
    }

    static public function endPage() {

        require A25X::PROJECT_CODE . '_HTML_END.php';
    }

}

class A25X_SQL extends CaseClass_A25_HTML {

    static public $SAMPLE_SQL = "SELECT WO,STN,在制數,良品轉入-良品轉出-當站報廢-待轉入-待轉出-分割转出-待完工-待PQC數 CHK_在制数,良品轉入, 良品轉出, 當站報廢, 委外加工數, 委外完成數, 待轉入, 待開工, 待完工, 待轉出,分割转出, 在制數, 待PQC數 FROM FT_SFCB_T_MORE WHERE 在制數<>良品轉入-良品轉出-當站報廢-待轉入-待轉出-分割转出-待完工-待PQC數";

    static public function isKeyword($val) {
//        echo "<h2>$val</h2>";
        $arrVal = explode(" ", trim($val));
//        var_dump($arrVal);
        $firstWord = strtoupper(trim($arrVal[0]));
        $keywords = array("SELECT", "FROM", "WHERE", "AND", "GROUP", "HAVING", "ORDER", "LEFT", "RIGHT", "JOIN", "ON", "UNION");

        if (in_array($firstWord, $keywords)) {
//            echo "$firstWord is found!<br>";
            return true;
        } else {
//             echo "$firstWord ???<br>";
            return false;
    ...
    static public function isHavingKeyword($val) {
//        echo "<h2>$val</h2>";
        $keywords = array("SELECT", "FROM", "WHERE", "AND", "GROUP", "HAVING", "ORDER", "LEFT", "RIGHT", "JOIN", "ON", "UNION");

        $arrVal = explode(" ", trim($val));
//        var_dump($arrVal);
        foreach ($arrVal as $key2 => $val2) {
            $firstWord = strtoupper(trim($val2));
            if (in_array($firstWord, $keywords)) {
//            echo "$firstWord is found!<br>";
                return true;
            }
    ...
    static public function getHighlightKeyword包括不在第一個字的($val) {
//        echo "<h2>$val</h2>";
        $arrVal = explode(" ", trim($val));
//        var_dump($arrVal);
...
//            echo "$firstWord is found!<br>";
    ...
    static public function getHighlightKeyword($val) {
//        echo "<h2>$val</h2>";
        $arrVal = explode(" ", trim($val));
//        var_dump($arrVal);

        $firstWord = strtoupper($arrVal[0]);
        $keywords = array("SELECT", "FROM", "WHERE", "AND", "GROUP", "HAVING", "ORDER", "LEFT", "RIGHT", "JOIN", "ON", "UNION");
...
//            echo "$firstWord is found!<br>";
    ...
    static public function getSQL客製格式顯示V4_fixLeftJoin($sql) {
        $sql = A25X_SQL::getSQL客製格式顯示V3($sql);
        return str_replace("<b>LEFT</b> <br><b>JOIN</b>", "<b>LEFT JOIN</b>", $sql);
    }

    static public function getExpandedSQL_highlightKeywords_fixLeftJoin($sql) {
//        $sql = "select OUT_DATE,STN, STN_NAME, SF161_CODE,REJ_DESC, SUM_REJ from A21_ASFT336_STN_REJ A LEFT JOIN DEV_REJ_CODE B ON A.SF161_CODE=B.REJ_CODE where OUT_DATE='2017-09' order by STN,SF161_CODE,SUM_REJ";
//        $sqlExpanded = "XXX";
//        A25X::showH4有灰底色($sql);
//        echo A25X::getSQL客製格式顯示V1($sql);
        $timer = 0;
        EXPANDING_HERE:
        $timer++;
//        A25X::showH4有淡红底色("timer=$timer");
//die();
//echo "<finally...timer=$timer";
//        echo "doing... $timer";
        $sqlExpanded = A25X::toExpandSQL($sql);
//A25X::showDEBUG("need to be aware SELECT (...)");
        if ($sqlExpanded != $sql) {
//    $timer++;
//            echo A25X::showH4有灰底色($sqlExpanded);
            $sql = $sqlExpanded;
            goto EXPANDING_HERE;
        } else {
//            echo A25X::showH4有灰底色("    %%%%% same %%%%, done!");
//            echo "finally...timer=$timer";
//            echo A25X::showH4有灰底色($sqlExpanded);
        }
        ...
        $sqlExpandedFormated = A25X_SQL::getSQL客製格式顯示V3($sqlExpanded);
        return str_replace("<b>LEFT</b> <br><b>JOIN</b>", "<b>LEFT JOIN</b>", $sqlExpandedFormated);
    }

    static public function getSQL客製格式顯示V3($sql) {
        $sql2 = str_replace(chr(10), '<br>', $sql);
        $arr = explode("<br>", $sql2);
        foreach ($arr as $key => $val) {
            $val2 = trim($val);
            if (strlen($val2) >= 2 and substr($val2, 0, 2) == '--') {
...
//            $result .= $val;
            if (A25X_SQL::isHavingKeyword($val)) {
//                $result .= '<br>' . str_replace('<br>', ' ', $val);
//                $val=A25X_SQL::getHighlightKeyword($val);
                $temp = str_replace('<br>', ' ', A25X_SQL::getHighlightKeyword包括不在第一個字的($val));
                $result .= str_replace('<_br>', '<br>', A25X_SQL::getHighlightKeyword包括不在第一個字的($temp));
            } else {
                $result .= str_replace('<br>', ' ', $val);
            }
        }
    ...
    static public function getSQL客製格式顯示V2($sql) {
        $sql2 = str_replace(chr(10), '<br>', $sql);
        $arr = explode("<br>", $sql2);
        foreach ($arr as $key => $val) {
            $val2 = trim($val);
        ...
        foreach ($arr2 as $key => $val) {
//            $result .= $val;
            if (A25X_SQL::isKeyword($val)) {
//                $result .= '<br>' . str_replace('<br>', ' ', $val);
//                $val=A25X_SQL::getHighlightKeyword($val);
                $result .= '<br>' . str_replace('<br>', ' ', A25X_SQL::getHighlightKeyword($val));
            } else {
                $result .= str_replace('<br>', ' ', $val);
            }
        }
    ...
    static public function getSQL客製格式顯示V1($sql) {
        $sql2 = str_replace(chr(10), '<br>', $sql);
        $arr = explode("<br>", $sql2);
        foreach ($arr as $key => $val) {
            $val2 = trim($val);
...
//            }
    ...
    // ONLY WORKING FOR THE FIRST FROM ...
    static public function toExpandSQLOnlyOneLevel($sql) {
        $arrForExpand = A25X_SQL::getArrayForExpandSQL();

        $sql2 = strtoupper($sql);
        $sql2Arr = explode("FROM", $sql2);
        if (isset($sql2Arr[1])) {
//            echo "<h1>" . $sql2Arr[1] . "</h1>";
            $sql3Arr = explode(" ", trim($sql2Arr[1]));
//            echo "<h2>" . $sql3Arr[0] . "</h2>";
            if (isset($sql3Arr[0]) && trim($sql3Arr[0]) != "(" && isset($arrForExpand[$sql3Arr[0]])) {
                echo "GOT IT " . $sql3Arr[0];
                return str_replace($sql3Arr[0], " ( " . $arrForExpand[$sql3Arr[0]] . " )", $sql);
            }
            return $sql;
        }
        return $sql;
    ...
    static public function showAscii($str) {
        echo "<br>";
    ...
    static public function getPossibleViewName($sql) {
        $arrForExpand = A25X_SQL::getArrayForExpandSQL();
//        var_dump($arrForExpand);
//        echo "<h2>$sql</h2>";
//        $sqlArr = explode(chr(10), strtoupper($sql));
        $sqlV1 = str_replace("<br>", " ", $sql);
        $sqlV2 = str_replace("<BR>", " ", $sqlV1);


        $sqlV3 = str_replace(chr(10), " ", $sqlV2);

//        $sqlV2=str_replace(chr(44), " ", $sqlV1);
//        $sqlV3=str_replace(chr(60), " ", $sqlV2);
        $sqlV99 = $sqlV3;
//        echo "<h2>$sqlV99</h2>";
//        die("DOING...");
        $sqlArr = explode(" ", strtoupper($sqlV99));
        foreach ($sqlArr as $key => $val) {
            $sqlArr2 = explode(" ", $val);
            foreach ($sqlArr2 as $key2 => $val2) {
                $trimmedVal = strtoupper(trim($val2));
                if (strlen($trimmedVal) > 0) {
//                      A25X::showAscii($trimmedVal);
//                    echo "#$trimmedVal#<br> ";
//                    if (in_array($trimmedVal ,$arrForExpand)){
//                        die("XXXXXXXXXXXXXXXXXXx  $trimmedVal");
//                    }
                    if (isset($arrForExpand[$trimmedVal])) {
//                         die("XXXXXXXXXXXXXXXXXXx  $trimmedVal");
                        return $trimmedVal;
                    }
                }
    ...
    static public function toExpandSQL($sql) {
        $arrForExpand = A25X_SQL::getArrayForExpandSQL();

        $sqlCaps = strtoupper($sql);

        $possibleViewName = A25X_SQL::getPossibleViewName($sqlCaps);
        if ($possibleViewName != '') {
//            A25X::showDEBUG("*** $possibleViewName");
            A25X::show有淡黄底色("展開 $possibleViewName<br>");

            return str_replace($possibleViewName, " ( " . A25X_SQL::getSQL拿掉註譯($arrForExpand[$possibleViewName]) . " )", $sql);
        }
        return $sql;
...
//        return $sql;
    ...
    static public function getArrayForExpandSQL() {

        $sql = " select VIEW_NAME, TEXT VIEW_DEF from USER_VIEWS 

        
        ORDER BY VIEW_NAME ";
//A25X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN("LIKE " . $data['view'], $sql, null);

        $arr = T100PROD::getArray($sql);
        foreach ($arr as $key => $val) {
            $arr2[$val['VIEW_NAME']] = A25X_SQL::getSQL拿掉註譯($val['VIEW_DEF']);
        }
//var_dump($arr);
        return $arr2;
    }

    static public function getSQL拿掉註譯($sql) {
        $sql2 = str_replace(chr(10), '<br>', $sql);
        $arr = explode("<br>", $sql2);
        foreach ($arr as $key => $val) {
            $val2 = trim($val);

            if (strlen($val2) >= 2 and substr($val2, 0, 2) == '--') {//只判斷 -- 是在新行開頭, 但不含在後段
            } else {
                $tempArr = explode("--", $val2);
                if (sizeof($tempArr) > 1) {
...
//             $arr2[]= explode("--", $val2)[0]."<br>";
        }

        $result = "";

        foreach ($arr2 as $key => $val) {
            if (strlen(trim($val)) > strlen("<br>")) {
                $result .= $val;
            }
        }
    ...
    static public function getSQL客製格式顯示($sql) {
        $sql2 = str_replace(chr(10), '<br>', $sql);
        $arr = explode("<br>", $sql2);
        foreach ($arr as $key => $val) {
            $val2 = trim($val);
...
class A25X extends A25X_SQL {

    static public function showT100數據庫_基本款有流水号_自动栏位($sql) {
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位($arr);
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL_是否顯示原欄位_EXT($sql, $toShowSQL = false, $toShowField = false) {
        ...
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位_EXT($arr, $toShowField);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL_是否顯示原欄位($sql, $toShowSQL = false, $toShowField = false) {

        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位($arr, $toShowField);
        if ($toShowSQL) {
            echo "$sql";
        }
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_是否顯示SQL($sql, $toShowSQL = false) {
        ...
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位_是否仍顯示原欄位($arr, $toShowSQL);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN($sql, $btn) {
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN($arr, $btn);
    }
...
//    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數($sql, $btn) {
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數($arr, $btn);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN($sql, $btn) {
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN($arr, $btn);
    }
...
//        echo  $title ;
        ...
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN_允許沒有BTN($arr, $btn);
    }

    static public function showNavDept($index_dept, $index_cat) {
        A25X::show("<h1>$index_cat  $index_dept</h1>");
        $sql = " select * from FT000 
                WHERE SQL='ENTRY' AND BTNS='$index_dept' AND SEQ=0
                ORDER BY TITLE
            ";
        if (A25X::isDevGroup()) {
            A25X::showDEBUG($sql);
        ...
        $arr = T100PROD::getArray($sql);
//    var_dump($arr);
//    APP, PAGE, TITLE, SQL, BTNS, SEQ, PROGRAMMER, UPPER, GRP
        foreach ($arr as $key => $val) {
            $rec = 1 + $key;
            $title = $val['TITLE'];
            $page = $val['PAGE'];
...
//            echo "<div class='ft_nav2'><a " . A25X::BTN_CLASS . " href='" . A25X::PROJECT_CODE . ".php?" . A25X::APP_IS_CASE . "&page=$page'>( $rec ) $title</a>"
//            . " $programmer</div> ";
            echo "<div class='ft_nav2'><a " . A25X::BTN_CLASS . " href='" . A25X::PROJECT_CODE . ".php?" . A25X::APP_IS_CASE . "&page=$page'>($rec) $title</a>"
            . " $programmer</div> ";
        }

//    A25X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN("入口列表", $sql, null);
    }

    static public function showNav() {

        $sql = "
SELECT BTNS DEPT , COUNT(*) CNT FROM (
select BTNS ,  TITLE,LINK from A25_FT000 
                WHERE SQL='ENTRY'
                  AND NOT BTNS IN ('HR系统整合','T100环境','保密等級')
                ORDER BY BTNS,APP, PAGE, SEQ 
                ) GROUP BY BTNS ORDER BY BTNS
        ";
...
////    A25X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN("ENTRY", $sql, null);
//        A25X::showT100數據庫_由左而右的LINK以BTN顯示("部門或模塊", $sql, null);
//    }
//
//    static public function showT100數據庫_由左而右的LINK以BTN顯示($title, $sql, $btn = null) {
//        echo  $title ;
//        if (A25X::isDevGroup()) {
//            echo "<h1>$sql</h1>";
//        }
//        echo "<div class='ft_nav'><span class='badge badge-default'>" . $title . "</span>&nbsp;&nbsp;";
        echo "<div class='ft_nav'>";
        $cat[1] = "（一）";
        $cat[2] = "（二）";
        $cat[3] = "（三）";
        $cat[4] = "（四）";
...
//            echo $val['LINK'] . " ";
            ...
            $dept = $val['DEPT'];
            $cnt = $val['CNT'];
            echo "<a " . A25X::BTN_CLASS . " href='" . A25X::PROJECT_CODE . ".php?&index_dept=$dept&index_cat=$catUpper'>$catUpper $dept ..$cnt</a> ";
        }
        if (A25X::isDevGroup()) {
            echo "<a class='btn btn-danger' href='http://10.10.0.100/cust001/case/a25_find_view.php?view=AQCT300'>SQL VIEW 查看</a>";
            echo "<a class='btn btn-success' href='http://10.10.0.100/cust001/case/a25_expand_view.php'>EXPAND VIEW </a>";
        }
        echo "<div>";
//        echo A25X::getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN_允許沒有BTN($arr, $btn);
    }
...
//            }
            A25X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN_BTNS用JSON字串_取FT000($app, $page, $seq);
        }
    }
...
//        $sql = X::getParsedSQL($arr[0]['SQL']);
        if (!isset($arr[0]['TITLE'])) {
            A25X::showH4有淡红底色("沒有設定 TITLE!");
            $sql = "SELECT* FROM FT000 WHERE APP='$app' AND PAGE='$page' ORDER BY SEQ ";
            A25X::showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN("FT000", $sql);
        } else {
            $title = $arr[0]['TITLE'];
            A25X::showH3("≡≡≡ " . $title . " ≡≡≡");
        }
    ...
    static public function getProgrammer($app, $page) {
        $sql = "SELECT* FROM FT000 WHERE APP='$app' AND PAGE='$page' AND SEQ='0'";
        $arr = T100PROD::getArray($sql);
//$data=X::getUrlData();
//        $sql = X::getParsedSQL($arr[0]['SQL']);
        $programmer = (isset($arr[0]['PROGRAMMER'])) ? $arr[0]['PROGRAMMER'] : "???";
        return $programmer;
    }

    static public function getRemarks($app, $page) {
        $sql = "SELECT* FROM FT000 WHERE APP='$app' AND PAGE='$page' AND SEQ='0'";
        $arr = T100PROD::getArray($sql);
//$data=X::getUrlData();
//        $sql = X::getParsedSQL($arr[0]['SQL']);
        $programmer = (isset($arr[0]['REMARKS'])) ? $arr[0]['REMARKS'] : "???";
        return $programmer;
    }

    static public function getFT000Grp($app, $page) {
        $sql = "SELECT* FROM FT000 WHERE APP='$app' AND PAGE='$page' AND SEQ='0'";
        $arr = T100PROD::getArray($sql);
//$data=X::getUrlData();
//        $sql = X::getParsedSQL($arr[0]['SQL']);
        $grp = (isset($arr[0]['GRP'])) ? $arr[0]['GRP'] : "???";
        return $grp;
    }
    ...
    static public function isDevGroup() {
        ...
        static public function isExcelGroup() {
        global $arrEnv;
        $result = $arrEnv[V::$USER_GROUP] == 'EXCEL' ? true : false;
        return $result;
    }
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_加立即查看BTN_多參數_多BTN_允許沒有BTN_BTNS用JSON字串_取FT000($app, $page, $seq) {
        ...
        $isBPM = false;
        $BPM = '【BPM】';
        $sql = A25X::getParsedSQL($arr[0]['SQL']);
...
//        echo "<span class='badge badge-default'>$title</span>";
        A25X::showH4有灰底色("($seq) $title");
//        A25X::showH4有灰底色("$title");


        if (A25X::isDevGroup()) {
            A25X::show有灰底色("$sql<br>");
            echo A25X::getExpandedSQL_highlightKeywords_fixLeftJoin($sql);
        }
        if ($isBPM) {
    ...
    static public function showT100數據庫_基本款有流水号_自动栏位_客制思科組產品BTN($sql) {
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位_客制思科組產品BTN($arr);
    }

    static public function showT100數據庫_基本款有流水号_自动栏位_客制視圖BTN($sql) {
        $arr = T100PROD::getArray($sql);
        echo A25X::getHtmlTable基本款有流水号_自动栏位_客制視圖BTN($arr);
    }
...
//            echo $alignX."  $align<hr>";
                ...
                switch ($alignX) {
                    ...
                    case 'R':
                        $strTable .= "<td class='text_align_right'>";
                        if (gettype($val2) == "integer" || gettype($val2) == "double") {
                            $strTable .= number_format($val2);
                        }else{
                            $strTable .= $val2;
                        }
            ...
            $PROJECT_CODE = A25X::PROJECT_CODE;
//            2017-11-13 10:10
            $x = '';
//            var_dump($btns);
            if ($btns == null) {
...
//                        A25X::PROJECT_CODE
                        $x .= "<a " . A25X::BTN_CLASS . " href='$PROJECT_CODE.php?app=case&page=$url'>$display</a> ";
...
//                            str_replace("world","Shanghai","Hello world!");
                        ...
                        $x .= "<a " . A25X::BTN_CLASS . " href='$PROJECT_CODE.php?app=case&page=$url$strPara'>$display</a> ";
//                        $x .= "<a class='' href='a23.php?app=case&page=$url$strPara'>$display</a> ";
                        ...
                        $x .= "<a " . A25X::BTN_CLASS . " href='$url$strPara'>$display</a> ";
                    }
                }
                $strTable .= "<td>$x</td>";
            }
    ...
    static public function getHtmlTable基本款有流水号_自动栏位__加立即查看BTN_多參數_多BTN_允許沒有BTN_有TITLE($title, $arr, $btns) {
        ...
        if (A25X::isDevGroup()) {
            A25X::showH4有淡红底色("col cnt =" . sizeof($arr));
        }
...
//                HTML::showDEBUG();
                ...
                foreach ($btns as $btn) {
                    ...
                    if (!isset($btn['para']) || $btn['para'] == null) {
                        ...
                        $x .= "<a class='btn btn-default' href='$PROJECT_CODE.php?app=case&page=$url'>$display</a> ";
...
//                            str_replace("world","Shanghai","Hello world!");
                        ...
                        $x .= "<a class='btn btn-default btn-sm' href='$PROJECT_CODE.php?app=case&page=$url$strPara'>$display</a> ";
//                        $x .= "<a class='' href='a23.php?app=case&page=$url$strPara'>$display</a> ";
                    } else {
                        $url = $btn['url'];
                        $display = $btn['display'];
...
//            echo $alignX."  $align<hr>";
                ...
                $strTable .= A25X::getAlignTd($alignX, $val2);
//                switch ($alignX) {
//                    case 'L':
//
//                        $strTable .= "<td>";
...
class 每日产出 {
    ...
    public static function showBtns() {
        A25X::showBtn("day_dept_stn_out.php?show_type=0&out_date=" . DEV::getToday(), " btn-primary", "【1】报工单");
        A25X::showBtn("day_dept_stn_batch.php?show_type=0&out_date=" . DEV::getToday(), " btn-primary", "【2】批号");
        A25X::showBtn("day_dept_stn_wo.php?show_type=0&out_date=" . DEV::getToday(), "  btn-primary", "【3】工单");
        A25X::showBtn("day_dept_stn_prod.php?show_type=0&out_date=" . DEV::getToday(), " btn-primary", "【4】产品");
        echo '&nbsp';
        A25X::showBtn("day_dept_stn_out.php?show_type=1&out_date=" . DEV::getToday(), "btn-success ", "【1】报工单+");
        A25X::showBtn("day_dept_stn_batch.php?show_type=1&out_date=" . DEV::getToday(), "btn-success ", "【2】批号+");
        A25X::showBtn("day_dept_stn_wo.php?show_type=1&out_date=" . DEV::getToday(), "btn-success ", "【3】工单+");
        A25X::showBtn("day_dept_stn_prod.php?show_type=1&out_date=" . DEV::getToday(), "btn-success ", "【4】产品+");
    }
...
//        echo HTML::getColorBtn(1, HTML::get全制程查询Url(), TITLE::$全制程查询);
        A25X::showBtn(A25X::get全制程查询Url(), " btn-primary", TITLE::$全制程查询);

        A25X::showBtn("qry001_wo_rej.php?show_type=0 ", " btn-info", "（1）基础信息");
        A25X::showBtn("qry001_wo_rej.php?show_type=1 ", " btn-info", "（2）显示单据");
        A25X::showBtn("qry001_wo_rej.php?show_type=2 ", "  btn-info", "（3）工站不良说明");
    }
...
class 批次不良 {
    ...
    public static function showBtns() {
        A25X::showBtn(A25X::get全制程查询Url(), " btn-primary", TITLE::$全制程查询);
        A25X::showBtn("qry001_batch_rej.php?show_type=0 ", " btn-info", "（1）基础信息");
        A25X::showBtn("qry001_batch_rej.php?show_type=1 ", " btn-info", "（2）显示单据");
        A25X::showBtn("qry001_batch_rej.php?show_type=2 ", "  btn-info", "（3）工站不良说明");
    }
...
class 图片开关 {
    ...
    public static function showBtns() {
        A25X::showBtn("helpdesk002.php?show_type=0 ", " btn-info", "（1）只显示文字");
        A25X::showBtn("helpdesk002.php?show_type=1 ", " btn-info", "（2）显示文字和图片");
    }
...
class 不合格品处理单 {
    ...
    public static function showBtns() {
        A25X::showBtn(A25X::get不合格品处理单161Url(), " btn-primary", "【161】SF161不合格品处理单");
        A25X::showBtn(A25X::get不合格品处理单162Url(), "btn-success ", "【162】SF162数量差异调整--不走签核");
    }
...
////        $btnClassV2 = " class='btn $btnClass '";
////        $url = "case_prod.php?PROD=$prod";
//        $url = A25X::get產品V2工单有带RcardUrl($prod);
//
//        return "<a href='$url'>$prod</a>";
//    }
//
...
////        $btnClassV2 = " class='btn $btnClass '";
////        $url = "case_asft301.php?wo=$wo";
//        $url = A25X::getAsft301RcardUrl($wo, $rcard);
//        return "<a href='$url'>$rcard</a>";
//    }
//
//    public static function getAsft301Url($wo) {
...
//    public static function get工单入库明细Link($wo, $text) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get工单入库明细Url($wo);
//        return "<a href='$url'>$text</a>";
//    }
//
//    public static function get按產品查工單Link($prod, $text) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get按產品查工單Url($prod);
//        return "<a href='$url'>$text</a>";
//    }
//
//    public static function get工單狀態統計表Url() {
...
//    public static function get客户订单查询V2Link($so, $cust, $so_yyyy_mm, $text) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get客户订单查询V2Url($so, $cust, $so_yyyy_mm);
//        return "<a href='$url'>$text</a>";
//    }
//
//    public static function get基础数据Url($url, $wo) {
...
//    public static function get工艺路线Link($url, $wo) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get工艺路线Url($url, $wo);
//        return "<a href='$url'>...工艺路线</a>";
//    }
//
//    public static function get工艺路线工单按批号项次工序良品转出的统计表Link($wo) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::getADV_WO_OUTUrl($wo);
//        return "<a href='$url'>...工艺路线及良品转出统计表</a>";
//    }
//
//    public static function get工艺路线工单按批号项次工序良品转出的统计表_实施RCARD_Link($wo, $rcard) {
...
//    public static function getADV_CHECKing002Link($wo) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::getADV_CHECKing002Url($wo);
//        return "<a href='$url'>$wo</a>";
//    }
//
//    public static function get期初工单工艺路线Link($url, $wo) {
...
//    public static function getADV工单按批号项次工序良品转出的统计表Link($wo) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::getADV工单按批号项次工序良品转出的统计表Url($wo);
//        return "<a href='$url'>$wo</a>";
//    }
//
//    public static function getADV产品按工单项次工序良品转出的统计表Link($prod) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::getADV产品按工单项次工序良品转出的统计表Url($prod);
//        return "<a href='$url'>$prod</a>";
//    }
//
//    public static function get工单Url($wo) {
...
//    public static function get客户有自己的订购单号列表Link($cust, $text) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get客户有自己的订购单号列表Url($cust);
//        return "<a href='$url'>$text</a>";
//    }
//
//    public static function get客户有自己的订购单号统计表Link($text) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get客户有自己的订购单号统计表Url();
//        return "<a href='$url'>$text</a>";
//    }
//
//    public static function get客户每月订单列表Link($cust, $so_yyyy_mm, $text) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get客户每月订单列表Url($cust, $so_yyyy_mm);
//        return "<a href='$url'>$text</a>";
//    }
//
//    public static function getBPM流程审批Link($bpm, $cnt) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::getBPM流程审批Url($bpm);
//        return "<a href='$url'>$cnt</a>";
//    }
//
//    public static function getBPM流程审批按流程至关卡Url($bpm) {
...
//    public static function getBPM流程审批按流程至关卡Link($bpm, $text) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::getBPM流程审批按流程至关卡Url($bpm);
//        return "<a class='btn btn-default' href='$url'>$text</a>";
//    }
//
//    public static function getBPM流程审批按流程至个人Url($bpm) {
...
//    public static function getBPM流程审批按流程至个人Link($bpm, $text) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::getBPM流程审批按流程至个人Url($bpm);
//        return "<a class='btn btn-default' href='$url'>$text</a>";
//    }
//
//    public static function getBPM补充说明Url($bpm) {
...
//    public static function get按库位查产品数量Link($wh) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get按库位查产品总数Url($wh);
//        return "<a href='$url'>$wh</a>";
//    }
//
//    public static function get库位明细查询_按库位和产品Link($wh, $prod, $qty) {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get库存明细查询作业_按库位和产品Url($wh, $prod);
//        return "<a href='$url?wh=$wh&prod=$prod'>$qty</a>";
//    }
//
//    public static function get富钛库位列表Link() {
////        $btnClassV2 = " class='btn $btnClass '";
//        $url = A25X::get富钛库位列表Url();
//        return "<a href='$url'>库位</a>";
//    }
//
//    public static function get客户订单每月笔数Link($month) {
...
//
//    public static function getBPM流程审批个人需要签核的统计表Link($txt) {
//        $url = A25X::getBPM流程审批个人需要签核的统计表Url();
//        return "<a href='$url'>$txt</a>";
//    }
//
//    public static function getBPM流程审批至个人Link($empe, $txt) {
//        $url = A25X::getBPM流程审批至个人Url($empe);
//        return "<a href='$url'>$txt</a>";
//    }
//
//    public static function getBPM流程审批至流程及个人Url($empe) {
...
//
//    public static function getBPM流程审批至流程及个人Link($bpm, $empe, $txt) {
//        $url = A25X::getBPM流程审批至流程及个人Url($empe);
//        return "<a href='$url'>$txt</a>";
//    }
//
//    public static function showSpan($text, $class) {
...
////        echo "<h4>$text</h4>";
//
//        A25X::showH4("<a href='case_prodname.php'>產品</a>︰$text");
//    }
//
//    static public function getProd產品名稱Link() {
////        echo "<h4>$text</h4>";
...
//    static public function showH4工单和Link有灰底色($wo) {
////        echo "<h4><span class='dev001'>$text</span></h4>";
//        A25X::showH4有灰底色("工单：" . A25X::get工单Link($wo));
//    }
//
//    static public function getPost变量x或预设($x, $default) {
//////           htmlspecialchars($_POST["p1"])
...
//    static public function showH4外协厂商和Link有灰底色($wo) {
////        echo "<h4><span class='dev001'>$text</span></h4>";
//        A25X::showH4有灰底色("外协厂商：" . A25X::get工单Link($wo));
//    }
//
//    static public function showH4有灰底色($text) {
//        echo "<h4><span class='dev001'>$text</span></h4>";
...
//    static public function showDbTable($text) {
////        echo "<h4>$text</h4>";
//        A25X::showH4("DB TABLE：" . $text);
//    }
//
//    static public function startPage() {
////        $show_sql = MARK::initVarWithDefaultVal('show_sql', 0); //
//        session_start(); //2017-11-08, start to implement 
//        require 'a25_HTML_START.php';
//    }
//
//    static public function endPage() {
//        DEV::show查询时间();
...
//            $strTable .= HTML::getBtn(HTML::getAsft301Url($wo), 'btn-primary', $wo);
            $strTable .= A25X::getAsft301Link($wo);
            $strTable .= "</td>";
...
//            $strTable .= "..." . $val['WO'];
            $wo = $val['WO'];
//            $strTable .= HTML::getBtn("case_asft301.php?WO=$WO", 'btn-primary', $WO);
            $strTable .= A25X::getBtn(A25X::getAsft301Url($wo), 'btn-primary', $wo);
            $strTable .= "</td>";
...
//            $strTable .= "<th>" . (1 + $key) . "</th>";
            ...
            foreach ($val as $key2 => $val2) {
                ...
                if ($val2 == 'N') {
                    ...
                    $strTable .= A25X::get按工单查报工列表Link($val2);
//                    $strTable .= HTML::getProdLink($val2);
                    $strTable .= "</td>";
                } else if ($key2 == "PROD") {
                    $strTable .= "<td>";
                    $strTable .= A25X::get產品Link($val2);
                    $strTable .= "</td>";
                } else {
                    $strTable .= "<td>";
                    $strTable .= $val2;
...
//                    $strTable .= implode(", ", REGEX::getEnglishNumberAndChineseWords($val2));
                    ...
                    $arr = REGEX::getEnglishNumberAndChineseWords($val2);
                    foreach ($arr as $k1 => $v1) {
                        $strTable .= A25X::getBtn("case_prodname_find.php?find=$v1", "btn-default", $v1);
                    }
...
//                    $strTable .= HTML::getBtn("case_prod.php?PROD=$val2", "btn-primary", $val2);
                    $strTable .= A25X::getBtn(A25X::get產品V2工单有带RcardUrl($val2), "btn-primary", $val2);
                    $strTable .= "</td>";
                } else {
                    $strTable .= "<td>";
                    $strTable .= $val2;
...
//                    $so_yyyy_mm="2017-09";
                    ...
                    $strTable .= "<td class='text_align_right'>";
//                            $strTable .= number_format($val2);
                    $strTable .= A25X::get客户每月订单列表Link($cust, $so_yyyy_mm, number_format($val2));
//                    $strTable .= $val2;
        ...
        foreach ($arr as $key => $val) {
            ...
            foreach ($val as $key2 => $val2) {
                if ($key2 == 'SO') {
                    $strTable .= "<td>";
//                    $strTable .= "XXXXXXXXXXXXXXXXXX";
                    $strTable .= A25X::get客户订单查询Link($val2);
//                    $strTable .= "XXXXXXXXXXXXXXXXXX";
//                    $cust=$val2['CUST'];
